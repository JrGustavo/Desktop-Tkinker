...
class ComponentesVentana(tk.Tk):
    ...
    def _crear_componentes_tabulador4(self, tabulador):
        imagen = tk.PhotoImage(file='python_logo.png')
        def mostrar_titulo():
            messagebox.showinfo('Mar info imagen', f'Nombre imagen: {imagen.cget("file")}')
    ...
    def _crear_componentes_tabulador5(self, tabulador):
        ...
        barra_progreso.grid(row=0, column=0, padx=10, columnspan=4)
        ...
        boton_detener = ttk.Button(tabulador, text='Detener ejecuciòn', command=detener )
        boton_detener.grid(row=1, column=2)
        boton_despues = ttk.Button(tabulador, text='Detener Ejecuciòn despúes', command=detener_despues)
